...
const doUpdateProducto = async(ev,setProductos, updateProducto,producto,removeSession) => {
  const {producto_id,codigo_producto,proveedor_id,nombre,stock,precio} = producto;
  if(producto_id && codigo_producto && proveedor_id && nombre && stock && precio){  
      ev.preventDefault();
      
      const producto_dto  = {codigo_producto,proveedor_id,nombre,stock,precio}
...
const doCreateProducto = async(ev,setProductos,updateProducto, producto,removeSession) => {
  const {codigo_producto,proveedor_id,nombre,stock,precio} = producto;
  
  if(codigo_producto && proveedor_id && nombre && stock && precio){  
      ev.preventDefault();
...
const mapToDispatchToProps = (dispatch) => {
  return {
      ...
      updateProductoIdProveedor : (producto,value) => {
          const newProductoState = {...producto};
